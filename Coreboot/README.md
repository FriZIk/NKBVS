...
26.07.2021]
1. Очень хочу сегодня сбросить это задание, из конкретных ошибок есть утечка памяти вот эта и переключение окон по 2 раза тыкая, но блин, я не знаю как это фиксить, просто уже устал.

27.08.2021
1. Сегодня сдам задачу, делаю очтёт и всё
2. Немного ещё код подправил добавил чтобы esc работал, сохранял данные и делал ребут, вот дополнительный код условия:
```
	case 27: // ESC
	...
	break;
```
И функционал в конфигураторах новый появился):
```
mvwprintw(w, getmaxy(w) - 3, 1, "2. Save configuration and reboot (ESC)");
```
